...
class cegar_option(polytope.DefaultOptions):
    def __init__(self):
        super().__init__()
        self.NumPolyToStartAbs = 2
        self.NumLayerToStartAbs = 4
        ...
        self.ReduceDupFacet = False # whether to reduce the H-resp
...
def poly_prop_from_layer(timeoutFile:str, pltp_mgr : polytope.polytopes, start_layer : int, init_polys : List[polytope.convex_polytope] , \
    ...
    with open('cegar_stack.log', 'a+') as stack_log:
        stack_log.write(str(cegar_stack)+'\n')
    ...
    for layer_idx in range(start_layer,stimulus_total+1):
        ...
        per_layer_option.DoJoin = len(prev_layer_polys) > options.NumPolyToStartAbs or layer_idx >= options.NumLayerToStartAbs
        ...
        if per_layer_option.Invtest and fp_ratio < options.PercentageOfFixedpointToContinue:
    ...
    print ('pos / neg / unknown / none = %d / %d / %d / %d' % (npos, nneg, nunknown, nnone))
    ...
    cegar_jumpto_poly_list = []
    cegar_jumpto_poly_layer = []
    for idx,status in enumerate(last_layer_status):
        poly.dump_to_file("polys/%d"%idx)
        if status.prev == polytope.PolyPrev.UNKNOWN:
            assert (poly.attribute.is_abstract)
            assert (poly.attribute.abs_poly_ref is not None)
            if poly.attribute.abs_poly_ref not in cegar_jumpto_poly_list:
                cegar_jumpto_poly_list.append(poly.attribute.abs_poly_ref)
                cegar_jumpto_poly_layer.append(poly.attribute.abs_poly_layer)

    print ('prevpolys:')
    for idx,nlayer in enumerate(cegar_jumpto_poly_layer):
        print (nlayer)


    with open('cegar_stack.log', 'a+') as stack_log:
        stack_log.write(str(cegar_stack)+': jump to layers:' + str(cegar_jumpto_poly_layer) + '\n')

    for idx,new_start_layer in enumerate(cegar_jumpto_poly_layer):
        new_start_poly = cegar_jumpto_poly_list[idx]
        res = \
            poly_prop_from_layer(pltp_mgr = pltp_mgr, start_layer = new_start_layer, init_polys=[new_start_poly],\
...
def test(ilb, iub, cegarOptions, expect_output, timeoutFile):
    ...
    with open('cegar_stack.log','w') as fout:
        fout.write('-------------\n') # clear the file
...
def run_all_tests(tests, timeout, timeoutFile):
    global maxlayers_gone
    global maxNeural
    cegarOptions = cegar_option()
    with open('cegar_test.log', 'w') as fp:
...
if __name__ == "__main__":
    #run_all_tests(tests = testranges[9:], timeout=100*60, timeoutFile='fp_test.lock')
    run_all_tests(tests = testranges, timeout=100*60, timeoutFile='cegar_test.lock')
